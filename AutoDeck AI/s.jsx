...
const SlideGenerator = ({ slides: initialSlides, config, tweaks, onBack }) => {
  ...
  // Quidax-first; secondary themes still tasteful but always
  // route the accent through QX.lime when it's the "purple" set.
  const THEMES = {
  ...
  const issueNum = String(deckTitleRaw.length % 90 + 10).padStart(2, '0');
  ...
  const SlideFrame = ({ t, hasImg, index, total, isLight }) => {
    ...
    return (
      <>
        {/* Top hairline + Quidax mark + section + slide number */}
  ...
  const SlideContent = ({ slide, t, layout, align, bgImg, index, total, gridOn }) => {
    ...
    if (layout === 'bigTitle') return (
      <div style={container}>
        ...
        <div style={{ ...bodyInset, flexDirection: 'column', justifyContent: 'flex-end', alignItems: ai }}>
          ...
          {slide.bullets[0] &&
          <div style={{ marginTop: 24, color: tx, fontFamily: qxType.body, fontSize: 'clamp(14px,1.55vw,18px)', lineHeight: 1.55, maxWidth: '62%', textAlign: ta }}>
              {slide.bullets[0]}
            </div>
          }
        </div>
    ...
    if (layout === 'stat') {
      // pull a number from the first bullet, or fallback
        ...
        <div style={container}>
          ...
          <div style={{ ...bodyInset, gap: 48, alignItems: 'center' }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 18, borderLeft: `1px solid ${t.rule}`, paddingLeft: 36 }}>
              ...
              {slide.bullets[1] &&
              <div style={{ paddingTop: 14, borderTop: `1px solid ${t.rule}`, fontFamily: qxType.body, fontSize: 'clamp(12px,1.25vw,14px)', color: tx, opacity: 0.85, lineHeight: 1.5 }}>
                  {slide.bullets[1]}
                </div>
              }
            </div>
  ...
  return (
    ...
    onClick={() => {setShowMenu(false);setShowThemePop(false);}}>
      ...
      onClick={(e) => e.stopPropagation()}>
        ...
        {/* Masthead — issue mark + deck title */}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {/* Theme picker (in popover) */}
          <div style={{ position: 'relative' }}>
            <button onClick={(e) => {e.stopPropagation();setShowThemePop((p) => !p);setShowMenu(false);}} style={chromeBtn(showThemePop)}>
              <span style={{ width: 14, height: 14, borderRadius: '50%', background: theme.swatch, border: '1px solid rgba(246,241,251,0.3)' }} />
              Theme
            ...
            {showThemePop &&
            <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, background: '#170729', border: '1px solid rgba(246,241,251,0.10)', borderRadius: 12, padding: 14, width: 240, zIndex: 300, boxShadow: '0 18px 50px rgba(0,0,0,0.6)' }} onClick={(e) => e.stopPropagation()}>
                ...
                <button key={k} title={t.name} onClick={() => {setGlobalTheme(k);}} style={{
                  aspectRatio: '1', borderRadius: 8, padding: 0, cursor: 'pointer',
                  background: t.gradient,
                  border: globalTheme === k ? '2px solid #F6F1FB' : '1px solid rgba(246,241,251,0.10)',
                  transition: `all 140ms ${qxEase}`
                }} />
                )}
                </div>
          ...
          {/* Customise — primary editor action */}
          ...
          {/* Present — lime CTA */}
          <button onClick={handlePresent} style={{
            height: 32, padding: '0 14px', borderRadius: 8, border: 'none',
            background: QX.lime, color: '#1A0530',
            fontFamily: qxType.body, fontSize: 12.5, fontWeight: 600,
            cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7,
            transition: `all 160ms ${qxEase}`,
            boxShadow: '0 6px 22px rgba(212,255,63,0.30)'
          }}
          ...
          <div style={{ position: 'relative' }}>
            ...
            {showMenu &&
            <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, background: '#170729', border: '1px solid rgba(246,241,251,0.10)', borderRadius: 12, padding: 6, minWidth: 230, zIndex: 300, boxShadow: '0 18px 50px rgba(0,0,0,0.6)' }} onClick={(e) => e.stopPropagation()}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', position: 'relative' }}>
        ...
        onClick={() => {setShowMenu(false);setShowThemePop(false);}}>
          ...
          <div style={{ width: '100%', maxWidth: editPanelOpen ? 880 : 1040, position: 'relative', transition: `max-width 240ms ${qxEase}` }}>
            ...
            {/* Slide caption strip — micro-interactions live here */}
            <div style={{ marginTop: 14, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ fontFamily: qxType.mono, fontSize: 10, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(246,241,251,0.40)' }}>
                  Layout
                </span>
                <span style={{ fontFamily: qxType.body, fontSize: 12, color: 'rgba(246,241,251,0.75)' }}>
                  {LAYOUTS.find((l) => l.key === layout)?.name || 'Standard'}
                </span>
                <span style={{ width: 3, height: 3, borderRadius: '50%', background: 'rgba(246,241,251,0.25)' }} />
                <span style={{ fontFamily: qxType.mono, fontSize: 10, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(246,241,251,0.40)' }}>
        ...
        onClick={(e) => e.stopPropagation()}>
            ...
            <button key={tab} onClick={() => setEditTab(tab)} style={{
              ...
              fontFamily: qxType.body, fontSize: 12,
              background: editTab === tab ? 'rgba(212,255,63,0.10)' : 'transparent',
              color: editTab === tab ? '#D4FF3F' : 'rgba(246,241,251,0.5)',
              fontWeight: editTab === tab ? 600 : 500,
              transition: `all 140ms ${qxEase}`, textTransform: 'capitalize'
            }}>{tab}</button>
            )}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                  ...
                  <button key={a.val} onClick={() => applyAlign(a.val)} style={{
                    flex: 1, height: 34, borderRadius: 7, padding: 0,
                    border: align === a.val ? '1px solid rgba(212,255,63,0.55)' : '1px solid rgba(246,241,251,0.08)',
                    background: align === a.val ? 'rgba(212,255,63,0.10)' : 'rgba(246,241,251,0.02)',
                    color: align === a.val ? '#D4FF3F' : 'rgba(246,241,251,0.55)',
                  ...
                  <div>
                    ...
                    return (
                      <button key={l.key} onClick={() => applyLayout(l.key)} style={{
                        ...
                        border: active ? '1px solid rgba(212,255,63,0.55)' : '1px solid rgba(246,241,251,0.08)',
                        background: active ? 'rgba(212,255,63,0.06)' : 'rgba(246,241,251,0.02)',
                        transition: `all 140ms ${qxEase}`,
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                placeholder="Search — business, finance, city…"
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                      {imgResults.map((img) =>
                <button key={img.id} onClick={() => applyImage(img.src)} style={{
                  padding: 0,
                  border: slideImages[currentIndex] === img.src ? '1px solid rgba(212,255,63,0.55)' : '1px solid rgba(246,241,251,0.08)',
                  borderRadius: 7, overflow: 'hidden', cursor: 'pointer', aspectRatio: '16/9', background: '#0a0118'
                }}>
                  ...
                  <div>
                    ...
                    return (
                      <button key={key} title={t.name} onClick={() => applySlideTheme(key)} style={{
                        aspectRatio: '1', borderRadius: 8, background: t.gradient, padding: 0, cursor: 'pointer',
                        border: active ? `2px solid ${isOverride ? '#D4FF3F' : '#F6F1FB'}` : '1px solid rgba(246,241,251,0.10)',
                        transition: `all 140ms ${qxEase}`
                      }} />);
                    ...
                    {slideThemeOverrides[currentIndex] &&
                <button onClick={() => setSlideThemeOverrides((p) => {const n = { ...p };delete n[currentIndex];return n;})} style={{ width: '100%', marginTop: 10, padding: '7px', borderRadius: 7, border: '1px solid rgba(246,241,251,0.08)', background: 'transparent', color: 'rgba(246,241,251,0.55)', fontFamily: qxType.mono, fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', cursor: 'pointer' }}>
                        Reset to deck theme
                      </button>
                }
                  </div>
            ...
            <div style={{ padding: '12px 18px', borderTop: '1px solid rgba(246,241,251,0.08)', flexShrink: 0 }}>
              <button onClick={() => {setEditPanelOpen(false);openAgent(currentIndex);}} style={{
              width: '100%', padding: '11px 14px', borderRadius: 9, border: '1px solid rgba(212,255,63,0.4)',
              background: 'rgba(212,255,63,0.08)', color: '#D4FF3F', fontFamily: qxType.body, fontSize: 12.5, fontWeight: 600, cursor: 'pointer',
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: `all 160ms ${qxEase}`
            }}
        ...
        {localSlides.map((s, i) => {
          ...
          return (
            <button key={i} onClick={() => goTo(i)} style={{
              flexShrink: 0, width: 124, padding: 0, cursor: 'pointer', position: 'relative',
              background: 'transparent', border: 'none', display: 'flex', flexDirection: 'column', gap: 5,
              transition: `all 160ms ${qxEase}`,
            ...
            onMouseLeave={(e) => {e.currentTarget.style.opacity = active ? 1 : 0.78;e.currentTarget.style.transform = 'translateY(0)';}}>
              <div style={{
                width: '100%', aspectRatio: '16/9', borderRadius: 5, overflow: 'hidden', position: 'relative',
                background: bgImg ? `url(${bgImg}) center/cover no-repeat` : t.gradient,
                border: active ? `1px solid ${QX.lime}` : '1px solid rgba(246,241,251,0.08)',
                boxShadow: active ? `0 0 0 1px ${QX.lime}30` : 'none'
              }}>
      ...
      <div style={{
        ...
        background: toast.type === 'success' ? 'rgba(52,199,123,0.14)' : toast.type === 'loading' ? 'rgba(212,255,63,0.10)' : 'rgba(23,7,41,0.94)',
        border: `1px solid ${toast.type === 'success' ? 'rgba(52,199,123,0.40)' : toast.type === 'loading' ? 'rgba(212,255,63,0.30)' : 'rgba(246,241,251,0.12)'}`,
        color: toast.type === 'success' ? '#6EE7B7' : toast.type === 'loading' ? '#D4FF3F' : '#F6F1FB',
        padding: '10px 18px', borderRadius: 24, fontFamily: qxType.body, fontSize: 12.5, zIndex: 500,
        backdropFilter: 'blur(12px)', boxShadow: '0 12px 40px rgba(0,0,0,0.5)',
        display: 'flex', alignItems: 'center', gap: 10, animation: `sgFadeUp 220ms ${qxEase}`
      }}>
      ...
      onClick={() => setAgentOpen(false)}>
        ...
        onClick={(e) => e.stopPropagation()}>
            ...
            <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                ...
                background: m.role === 'user' ? QX.lime : 'rgba(246,241,251,0.06)',
                color: m.role === 'user' ? '#1A0530' : 'rgba(246,241,251,0.92)',
                fontFamily: qxType.body, fontSize: 13, lineHeight: 1.5, fontWeight: m.role === 'user' ? 500 : 400
              }}>{m.text}</div>
                </div>
            )}
              {agentThinking &&
            ...
            style={{ flex: 1, padding: '10px 12px', borderRadius: 9, border: '1px solid rgba(246,241,251,0.10)', background: 'rgba(246,241,251,0.04)', color: '#F6F1FB', fontFamily: qxType.body, fontSize: 13, outline: 'none', resize: 'none', lineHeight: 1.45 }} />
              ...
              background: agentInput.trim() && !agentThinking ? QX.lime : 'rgba(246,241,251,0.06)',
              color: agentInput.trim() && !agentThinking ? '#1A0530' : 'rgba(246,241,251,0.30)',
              cursor: agentInput.trim() && !agentThinking ? 'pointer' : 'default',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              transition: `all 160ms ${qxEase}`
            }}>
